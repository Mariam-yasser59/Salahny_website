...
export default function DriverWorkshops() {
  ...
  const workshops = useMemo(() => {
    return toList(data)
      .map((workshop) => {
        ...
        const distance = distanceKm(driverLocation, workshopLocation);
  ...
  return (
    <div className="dash-stack">
      <SectionHeader title="Nearby Workshops">
        List and map-style marketplace for verified auto service partners.
      </SectionHeader>
        ...
        <div className="map-panel">
          ...
          {driverLocation ? (
            ...
            <>
              ...
              <button className="primary-btn" onClick={getMyLocation}>
                Use My Location
              </button>
            </>
          )}
            ...
            <div className="compact-card">
              ...
              <p>
                {nearestWorkshop.distanceKm != null
                  ? `${nearestWorkshop.distanceKm.toFixed(1)} km away`
              ...
              <article className="compact-card" key={id}>
                ...
                <p>
                  {typeof workshop.location === 'string'
                    ? workshop.location
                    : workshop.address || workshop.location?.address || 'No address available'}
                </p>

                <p>
                  <Star size={16} fill="currentColor" /> {workshop.rating || 4.8} ·{' '}
                  {workshop.distanceKm != null
                    ? `${workshop.distanceKm.toFixed(1)} km away`
                    : workshop.distance || 'Distance unavailable'}
                </p>

                <StatusBadge value={isOpen ? 'open' : 'closed'} />

                <button className="primary-btn" onClick={() => bookWorkshop(workshop)}>
                  Book
                </button>
              </article>
            );
